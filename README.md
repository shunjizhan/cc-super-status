...
Other constants (bar width `10` cells, transcript mtime lookback `max(CCSS_WINDOW, CCSS_ACTIVE_WINDOW) + 60s`, `1 MiB` tail read per transcript, projects dir `$HOME/.claude/projects`) are fixed in `src/config.ts`.
