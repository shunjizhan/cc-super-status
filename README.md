...
| `CCSS_CELLS` | `10` | ⚡ bar **width** in cells — the per-layer width, independent of the layer count. Each cell is 10% of one layer, so a wider bar just resolves each layer more finely (e.g. `CCSS_CELLS=20` draws each layer across 20 cells). It no longer sets the layer count — that's `CCSS_BAR_MODE` / your plan. |
