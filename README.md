...
| `CCSS_BAR_MODE` | `auto` | ⚡ bar mode. `max` → the long **4× bar that counts down from 400%** (40 cells); `default` (aka `normal`/`1x`) → the normal **100% bar** (10 cells); `auto` → detect from your plan (Max 20x → `max`, else `default`). The plan tier isn't on stdin, so `auto` reads it from `~/.claude.json` (`oauthAccount.organizationRateLimitTier`); any read failure falls back to `default`. Set `max`/`default` to force either mode regardless of plan. |
| `CCSS_CELLS` | *(mode)* | Exact ⚡ bar width in cells — the **top override**, beats `CCSS_BAR_MODE` and auto-detect. Each cell is a fixed **10%**, so the bar counts from `cells × 10`% (e.g. `CCSS_CELLS=20` → a 20-cell bar counting from 200% — a middle ground between the 10- and 40-cell bars). |
